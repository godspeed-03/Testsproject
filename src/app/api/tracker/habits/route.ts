...
export async function GET(req: Request) {
  try {
    ...
    let habits = await prisma.habitItem.findMany({ where: { userId } });
    habits = habits.map((h: any) => recalculateHabitStreak(h));

    let lists = await prisma.checkList.findMany({ where: { userId } });
      ...
      if (habit) {
        await prisma.habitItem.update({
          ...
          data: {
            ...
            startDate: startDate !== undefined ? startDate : habit.startDate,
            ...
            subject: subject !== undefined ? subject : habit.subject,
            topic: topic !== undefined ? topic : habit.topic,
