...
export async function POST(req: Request) {
  try {
    ...
    const formattedTestLogs = testLogs.map((item: any) => ({
      ...
      percent: item.percent !== undefined ? item.percent : Math.round(((item.score || 0) / (item.maxScore || 200)) * 100),
      correctCount: item.correctCount ?? (item.correct !== undefined ? item.correct : Math.round((((item.percent || Math.round(((item.score || 0) / (item.maxScore || 200)) * 100)) / 100) * 100) * 0.5)),
      incorrectCount: item.incorrectCount ?? (item.incorrect !== undefined ? item.incorrect : Math.round((((100 - (item.percent || Math.round(((item.score || 0) / (item.maxScore || 200)) * 100))) / 100) * 100) * 0.2)),
      unattemptedCount: item.unattemptedCount ?? (item.unattempted !== undefined ? item.unattempted : 30),
