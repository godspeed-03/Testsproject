...
import { useTracker, getTargetGoalLabel, calculateHabitStreak, isHabitScheduledForDate } from '../TrackerContext';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className={`text-lg sm:text-xl font-black ${textTitle}`}>Habits & Streaks</h2>
              ...
              <div key={h._id} className={`p-5 rounded-2xl border ${cardBg} space-y-4 shadow-xs`}>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      <h3 className={`font-black text-base sm:text-lg ${textTitle}`}>{h.title}</h3>
                      {h.description && <p className={`text-xs ${textMuted} mt-0.5`}>{h.description}</p>}
                    </div>
                  </div>

                  {/* Badges Strip */}
                  <div className="flex flex-wrap items-center justify-start md:justify-center gap-2 text-xs font-bold">
                    {isNumeric ? (
                      <span className={`px-2.5 py-1 rounded-lg border text-xs font-black ${
                        ...
                        🎯 Today: {todayVal} / {h.target?.value || 1} {h.target?.unit || 'times'}
                      </span>
                    ) : (
                ...
                {globalViewMode === 'month' ? (
                  <div className="pt-2 space-y-2">
                    <div className="flex items-center justify-between text-2xs font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-950 px-3 py-1.5 rounded-2xl border border-slate-200 dark:border-slate-800">
                          ...
                          <span className="text-xs font-black px-2 min-w-[90px] text-center text-slate-800 dark:text-slate-200 normal-case">
                            {mName} {mYear}
                          </span>
                          <button
                            type="button"
                      ...
                      <div className="flex items-center gap-3 text-[10px]">
                        <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold">
                          <span className="w-2.5 h-2.5 rounded-md bg-emerald-500" /> Done
                        </span>
                        <span className="flex items-center gap-1 text-rose-600 dark:text-rose-400 font-bold">
                          <span className="w-2.5 h-2.5 rounded-md bg-rose-500" /> Missed
                        </span>
                      </div>
                    </div>
                        ...
                        {weekDays.map((w: any) => {
                          ...
                          return (
                            <button
                              key={w.iso}
                              type="button"
                              disabled={!scheduled || isPast || saving}
                              onClick={() => handleItemClick(h, w.iso)}
                              className={`p-2 sm:p-2.5 rounded-xl text-center transition-all flex flex-col items-center justify-center gap-1 min-h-[64px] ${cardStyle}`}
                            >
