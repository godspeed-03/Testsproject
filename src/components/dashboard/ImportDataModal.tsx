...
export default function ImportDataModal({
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[999999] overflow-y-auto bg-slate-900/60 dark:bg-slate-950/85 backdrop-blur-md px-3 sm:px-4 py-8 text-center animate-fade-in">
      {/* Vertically align helper */}
      <span className="inline-block h-screen align-middle" aria-hidden="true">&#8203;</span>

      {/* Modal Dialog Card */}
      <div className="inline-block w-full max-w-xl text-left align-middle transition-all transform bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        
        {/* Modal Header */}
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/80 dark:bg-slate-900 space-y-3">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">Pick a file or paste JSON to seed topics, daily logs & subjects.</p>
            </div>
            <button type="button" onClick={onClose} className="text-slate-400 hover:text-rose-500 p-1 transition-colors">
              <X size={20} />
            </button>
          </div>
        ...
        {/* Body Content */}
        <div className="p-4 sm:p-6 space-y-4 max-h-[60vh] overflow-y-auto bg-white dark:bg-slate-900">
          ...
          {activeTab === 'upload' ? (
            <div className="space-y-3">
              <label className="block border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-indigo-500 dark:hover:border-indigo-400 bg-slate-50 dark:bg-slate-950/50 rounded-2xl p-6 sm:p-8 text-center cursor-pointer transition-all">
              ...
              {selectedFile && (
                <div className="bg-slate-100 dark:bg-slate-950 p-3 rounded-xl border border-slate-200 dark:border-slate-800 text-xs flex justify-between items-center">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <label className="text-xs font-extrabold text-slate-900 dark:text-slate-100">Paste Raw JSON Content</label>
                <button
                  type="button"
              ...
              <textarea
                ...
                className="w-full p-3 rounded-xl border text-xs font-mono font-medium bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 border-slate-300 dark:border-slate-800 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              />
            </div>
          )}
        </div>
        ...
        {/* Footer Actions */}
        <div className="p-4 sm:p-5 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 flex justify-between items-center gap-2 flex-wrap">
          {onExportJson ? (
            <button
              type="button"
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-slate-200 hover:bg-slate-300 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 text-xs font-extrabold rounded-xl transition-colors"
            >
              Cancel
            </button>
            <button
